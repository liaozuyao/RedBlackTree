 ...
 * @param <D>
 */
public class MyRBree<T extends Comparable<T>, D> {
    ...
    /**
     * 寻找待删除节点的后继节点
     * （这个节点即将被删除，所以要选个后继节点补到这个位置，
     * 选择节点的规则
     * 和普通二叉树一样，要么就是找左子树的最大值，要么就是右子树的最小值）
     *
     * @param node
     * @return
     */
    public RBNode successor(RBNode<T, D> node) {
        ...
        //下面不会进入， 只有node的两个孩子都不为null十才会进入
    ...
    /**
     * 对某个节点进行左旋
     * （当前节点就是父亲节点，整个过程就是父亲下沉，右孩子上升，然后把右孩子的左节点变成了原父亲的右节点）
     *
     * @param x
     */
    ...
    public void balanceInsertion(RBNode<T, D> node) {
        ...
        while (((parent = parentOf(node)) != null) && isRed(parent)) {
            ...
            if (grandparent.leftChild == parent) {
                ...
                RBNode<T, D> uncle = grandparent.rightChild;
                ...
                if (isRed(uncle)) {
                    ...
                    //因为叔父一开始就是红色，那说明祖父必为黑色，那祖父的兄弟也肯定是黑色，因为黑色的兄弟节点，必然同色
                    //进入一次之后，父亲变成了当前节点，原来祖父变成了父亲，那说明新父亲不会与新叔父同为红色
                    continue;
                } else {
                    //叔父是黑色，父亲为红色
                     ...
                     */
                    rightRonate(grandparent);
                }
            } else {
                //同理，先找到叔父
                RBNode<T, D> uncle = grandparent.leftChild;
                if (isRed(uncle)) {
                    //这个过程其实和上面的是一致的，不需要旋转，只需要重新染色即可
                    setBlack(parent);
                    setBlack(uncle);
                    setRed(grandparent);
                    node = grandparent;
                    ...
                    setBlack(parent);
                    setRed(grandparent);
                    leftRonate(grandparent);
                }
            }
        }
        //调整后，假如都调整到了根处了，必须再次检验，让根为黑色，让它继续满足红黑树的性质
        if (root == node) {
            setBlack(node);
        }
    }
    ...
    public void balanceDeletion(RBNode<T, D> node, RBNode<T, D> parent) {
        ...
        while (isBlack(node) && node != this.root) {
            //假如替代节点的颜色也是为黑，这里为什么要用‘也’，能进这个方法，肯定说明被删节点也是黑色
            //替代节点也是黑色，那肯定要做调整，不然整条路径，就减少了一个黑色节点，最终肯定是不符合红黑树条件
            if(parent.leftChild == node){
                //假如替代节点是其新父节点的左节点，那就通过右指针拿到其兄弟节点
                other = parent.rightChild;
                System.out.println("当前parent: " + parent.key + " other(兄弟节点):" + other.key);
                if (isRed(other)) {
                    //假如兄弟是红色， 那么父亲肯定是黑色
                    System.out.println("兄弟当前是红色");
                    System.out.println("进入balanceDetetion的while (情况2-L-a：兄弟是红色）");
                    ...
                    if(isBlack(other.leftChild) && isBlack(other.rightChild)){
                        //假如兄弟节点没有任何孩子节点，也会进入这个分支，因为叶子也相当于是黑色的
                        //other就是替换节点的兄弟节点
        ...
        //这里，node其实是即将放入被删除位置的替代节点
        //假如node是红色，那同时被删除节点是黑色
        //那说明，直接把node的颜色由红变黑，就直接满足，不需要做任何旋转
        if(node != null){
    ...
    public void insertNode(T key, D data){
        ...
        RBNode<T, D> y = null;
        //这个过程和二叉查找树的过程是一样的，从上循环到底，知道找到为止
        while (x != null){
            y = x;
    ...
    public void printTreeLevel() {
        ...
        for (int i = 0; i < map.size(); i++) {
            ...
            for (int j = 0; j < map.get(i).size(); j++) {
                System.out.println(makeSpace2(size, i) +
    ...
    public void showTree(RBNode root, int count, ConcurrentHashMap<Integer, List<RBNode>> map) {
        ...
        if (root.leftChild != null) {
            ...
            //假如为空，也添加到map中，因为要做格式化控制，空的，也要知道它这个位置是空的
